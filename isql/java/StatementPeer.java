...
public class StatementPeer
{
   ...
   */
  private void absolute(ResultSet rs)
    throws SQLException
  {
    if(offset != 0){
      rs.absolute(offset);
  ...
  {
    return self.conn.peer.supportsGetGenKeys && FanStr.startsWith(self.sql, "insert") ?
  ...
  private Object executeResult(Statement self, int rows)
  {
    try
    {
      if (self.conn.peer.supportsGetGenKeys && FanStr.startsWith(self.sql, "insert"))
      {
  ...
  public long offset(Statement self)
  {
    return offset;
  }
  public void offset(Statement self, long offset)
  {
